...
pub fn init(
    ...
    const vert_path = try std.fmt.bufPrint(&vert_path_buf, shader_path_format, .{ ui_shader_name, "vert" });
    ...
    const frag_path = try std.fmt.bufPrint(&frag_path_buf, shader_path_format, .{ ui_shader_name, "frag" });
    ...
    const descriptor_types = [vulkan_types.ui_shader_descriptor_count]vk.DescriptorType{
        .uniform_buffer,
    ...
    const material_ubo_pool_sizes = [_]vk.DescriptorPoolSize{
        vk.DescriptorPoolSize{
            .type = .uniform_buffer,
            .descriptor_count = vulkan_types.ui_shader_instance_max_count,
        },
        vk.DescriptorPoolSize{
            .type = .combined_image_sampler,
            .descriptor_count = vulkan_types.ui_shader_sampler_count * vulkan_types.ui_shader_instance_max_count,
        },
    };
    ...
    self.material_descriptor_pool = try context.device_api.createDescriptorPool(
        ...
        &vk.DescriptorPoolCreateInfo{
            ...
            .max_sets = vulkan_types.ui_shader_instance_max_count,
        },
        null,
    );
    ...
    self.material_uniform_buffer = try Buffer.init(
        context,
        @sizeOf(UIInstanceUniformData) * vulkan_types.ui_shader_instance_max_count,
        .{ .transfer_dst_bit = true, .uniform_buffer_bit = true },
        .{
            .device_local_bit = self.context.physical_device.supports_local_host_visible,
            .host_visible_bit = true,
...
pub fn applyMaterial(self: *UIShader, material: MaterialHandle) !void {
    ...
    var descriptor_writes: [vulkan_types.ui_shader_descriptor_count]vk.WriteDescriptorSet = undefined;
